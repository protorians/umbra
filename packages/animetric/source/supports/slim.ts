...
import parseSequence = TextUtility.parseSequence;
...
export function createSlimSequence<T extends HTMLElement>(
    ...
    const initial: (CSSStyleDeclaration[keyof CSSStyleDeclaration])[] = []
...
export function createSlimSequences<T extends HTMLElement>(
    ...
    for (const property of Object.keys(to)) {
        ...
        timelines.push(createSlimSequence(target, property as keyof CSSStyleDeclaration, _from, _to, forwarding))
    }
    return timelines;
}
